...
def obter_usuarios(
    ...
    return admin_service.obter_usuarios(
        usuario=usuario, offset=offset, limit=limit, sessao=sessao
    )
...
def obter_usuario(
    ...
    return admin_service.obter_usuario(
        id_usuario=id_usuario, usuario=usuario, sessao=sessao
    )
...
def atualizar_usuario(
    ...
    return admin_service.atualizar_usuario(
        id_usuario=id_usuario, usuario=usuario, dados=dados, sessao=sessao
    )
...
@admin_rota.delete(
    path='/usuarios/{id_usuario}', status_code=status.HTTP_204_NO_CONTENT
)
def deletar_usuario(
    id_usuario: int,
    usuario: Usuario = Depends(verificar_token),
